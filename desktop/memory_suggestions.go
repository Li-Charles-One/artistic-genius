...
func workflowCategories() []workflowCategory {
	return []workflowCategory{
		...
		{
			Name:        "artistic-genius-memory-ui",
			Description: "Iterate on the Artistic Genius desktop Memory page with source-backed UI decisions and browser verification.",
...
func skillCandidateBody(cat workflowCategory, evidence []string) string {
	var b strings.Builder
	title := strings.TrimPrefix(strings.ReplaceAll(cat.Name, "-", " "), "artistic-genius ")
	b.WriteString("# " + cases.Title(language.Und).String(title) + "\n\n")
	b.WriteString("Use this skill when the user asks for this repeated Artistic Genius workflow.\n\n")
