...
// by substring (file names embed the key, e.g. Artistic Genius-darwin-arm64.zip), so the
...
// (artistic-genius.key) and the public key (artistic-genius.pub) into dir. The password comes
...
func genKey(dir string) error {
	...
	keyPath := filepath.Join(dir, "artistic-genius.key")
	pubPath := filepath.Join(dir, "artistic-genius.pub")
...
func signFiles(files []string) error {
	...
	for _, f := range files {
		...
		sig := minisign.SignWithComments(priv, data,
			"file:"+filepath.Base(f), "Artistic Genius desktop release")
		out := f + ".minisig"
...
func genManifest(dir, version, tag string) error {
	...
	if repo == "" {
		return fmt.Errorf("manifest: GITHUB_REPOSITORY is not set")
	}
	m := update.Manifest{
		Version:      version,
