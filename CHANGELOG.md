...
All notable changes to Artistic Genius are recorded here.
...
- **Code intelligence via CodeGraph**: a tree-sitter symbol/call graph
  ...
  or API cost. Fetched into a local cache on first use (or `artistic-genius codegraph
...
- **Memory**: `ARTISTIC_GENIUS.md` hierarchy + auto-memory, folded into the cache-stable
  prefix.
- **ACP** (`artistic-genius acp`) and an HTTP/SSE server frontend; desktop app (Wails).
...
- Versions: the local reset starts from the Go desktop codebase.
