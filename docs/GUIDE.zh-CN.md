...
优先级：**flag > `./artistic-genius.toml` > 用户配置文件 > 内置默认值**。用户配置位于操作系统配置目录：
Linux 为 `~/.config/artistic-genius/`，macOS 为 `~/Library/Application Support/artistic-genius/`，Windows 为 `%AppData%\artistic-genius\`。
...
# language    = "zh"               # 界面语言；为空则按 $LANG / $ARTISTIC_GENIUS_LANG 自动检测
...
桌面端快捷键在 **设置 → 快捷键** 中管理。选择一行后按下新的组合键，Artistic Genius 会为桌面端保存该绑定。
...
`artistic-genius chat` 会在 writer 调用前征求同意（普通工具为 `1` 本次 · `2` 本会话允许此范围 · `3` 总是允许此范围（保存） · `4` 拒绝；Bash 可额外选择命令前缀授权）；
其中 Bash 默认按具体命令记，也可按安全推导出的命令前缀记（如 `Bash(go test:*)`）；文件编辑类工具的本会话授权按编辑能力记，持久授权则写入 `Edit(<path>)` 文件路径规则；
`artistic-genius run` 保持自主运行但仍然遵守 `deny`。
...
Artistic Genius 是一个 MCP 客户端。`[[plugins]]` 的 `type` 选择传输：`stdio`（默认）启动本地子进
...
各自暴露的内容。`make build` 还会产出 `bin/artistic-genius-plugin-example`——一个可直接运行的
...
**已有 Claude Code 的 `.mcp.json`？** 直接放到项目根目录，Artistic Genius 会原样读取——其
`mcpServers` 规范（`command`/`args`/`env`、`type`/`url`/`headers`、`${VAR}` 展开）
与 `[[plugins]]` 字段一一对应。两处来源会合并加载；同名时以 `artistic-genius.toml` 为准。
...
**从 `0.x` 升级？** 旧的 `~/.artistic-genius/config.json` 仍会被读取（读其 `mcpServers`、并遵从
`mcpDisabled`），作为最低优先级来源——所以 MCP 服务器照常可用；方便时再把它们挪进
`artistic-genius.toml` 的 `[[plugins]]` 或 `.mcp.json`。

## 斜杠命令

`artistic-genius chat` 里，内置命令（`/compact`、`/new`、`/clear`、`/rewind`、`/tree`、`/branch`、`/switch`、`/todo`、`/model`、`/mcp`、`/skills`、`/hooks`、`/memory`、`/output-style`、`/sandbox`、`/language`、`/auto-plan`、`/reasoning-language`、`/help`）在本地执行——`/help` 可列出全部。
...
是放在 `.artistic-genius/commands/`（项目）或 `~/.config/artistic-genius/commands/`（用户）下的 Markdown 文件——
`review.md` 即 `/review`，子目录构成命名空间（`git/commit.md` → `/git:commit`）。文件正文
是 prompt 模板，调用即作为一轮对话发出。

`/memory` 会同时列出记忆文档（`ARTISTIC_GENIUS.md` / `AGENTS.md`）和已保存的 auto-memory 条目。
...
在消息里写 `@` 引用，Artistic Genius 会在发送前解析成带标签的上下文块：`@path/to/file`（或
...
`artistic-genius setup` 刻意保持首次体验极简：选 provider → 输入 key（所选 provider 的所有
SKU 都会启用）。若要让两个模型协同（执行器 + 规划器，各自独立、缓存稳定的
session），向导后手动在 `artistic-genius.toml` 加一行即可：
...
Planner 会看到已加载的 `ARTISTIC_GENIUS.md` / `AGENTS.md` 记忆，并拿到一小组只读研究工具，
...
再写进项目的 `./artistic-genius.toml`，例如超大代码库需要更高的 planner 上限。
...
的任务会自动进入 plan mode：Artistic Genius 先只读生成计划，待用户批准后才
编辑文件或执行有副作用的命令。`auto_plan_classifier` 可以指定便宜的 provider，例如
`deepseek-flash`；它只在边界输入上调用，分类失败会回退到启发式规则。也可以用
`artistic-genius chat` 里的 `/auto-plan off|on` 修改用户级设置，或在 shell/脚本里用
`artistic-genius config auto-plan off|on`。可见思考语言也采用同样形态：chat 里用
`/reasoning-language auto|zh|en`，shell/脚本里用
`artistic-genius config reasoning-language auto|zh|en`。只有明确想写项目级覆盖时，才给
shell 命令加 `--local`。
