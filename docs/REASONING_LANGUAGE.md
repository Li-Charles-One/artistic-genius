...
Inside `artistic-genius chat`, use the slash command:
...
`artistic-genius.toml`; use the shell command with `--local` for that.

Headless runs also use the same setting:

```bash
artistic-genius run "explain this module"
```
...
```text
./artistic-genius.toml > user config.toml > built-in defaults
```
...
When set to `zh` or `en`, Artistic Genius adds a small transient
