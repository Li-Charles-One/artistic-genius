...
Hooks 让 Artistic Genius 在会话、用户输入、工具调用、模型返回、压缩上下文等节点执行本地 shell 命令。桌面端在“设置 -> Hooks”里提供图形化编辑入口，本质上读写同一份 `settings.json`。
...
1. 打开桌面端“设置 -> Hooks”。
2. 选择范围：
   - “全局”：保存到 `~/.artistic-genius/settings.json`，始终加载。
   - “项目”：保存到当前工作区的 `.artistic-genius/settings.json`，必须点击“信任此工作区”后才会加载。
...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "match": "bash",
        "command": "node .artistic-genius/hooks/check-bash.js",
    ...
    "Stop": [
      {
        "command": "echo Artistic Genius turn finished"
      }
    ]
  }
}
...
| 范围 | 文件 | 是否需要信任 | 加载顺序 |
| --- | --- | --- | --- |
| 全局 | `~/.artistic-genius/settings.json` | 不需要 | 项目 hooks 之后 |
| 项目 | `<workspace>/.artistic-genius/settings.json` | 需要 | 全局 hooks 之前 |

项目 hooks 的信任状态不写在项目文件里，而是写在用户自己的 `~/.artistic-genius/trust.json`。这样克隆来的仓库不能靠提交 `.artistic-genius/settings.json` 自动执行命令。
...
```json
{
  "hooks": {
    "PreToolUse": [
      { "match": "bash", "command": "node .artistic-genius/hooks/pre-tool.js" }
    ],
    "UserPromptSubmit": [
      { "command": "node ~/.artistic-genius/hooks/check-prompt.js" }
    ],
    "Stop": [
      { "command": "osascript -e 'display notification \"Turn done\" with title \"Artistic Genius\"'" }
    ]
  }
}
```
...
```json
[
  { "event": "PreToolUse", "match": "bash", "command": "node .artistic-genius/hooks/pre-tool.js" },
...
`command` 会通过平台 shell 执行：macOS/Linux 使用 `sh -c`，Windows 使用 `cmd /c`。stdin 是 Artistic Genius 写入的一行 JSON，见下面的 payload 表。
...
Artistic Genius 会把一行 JSON 写入 hook 命令的 stdin。所有 payload 都至少有：
...
- `PostLLMCall`：exit 0 且 stdout 非空时，stdout 会替换用户看到的 reasoning。若 provider 的 reasoning 带签名，Artistic Genius 会保留原始 signed reasoning 用于后续请求，同时仍展示 hook 转换后的文本。
- `PreCompact`：所有非空 stdout 会按换行拼接，作为本次压缩摘要的额外指导。
- 其它事件：stdout 只在非通过结果中作为提示文本使用，不会自动进入模型上下文。

## 示例：阻止危险 bash 命令

`.artistic-genius/settings.json`：
...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "match": "bash",
        "command": "node .artistic-genius/hooks/block-dangerous-bash.js",
...
`.artistic-genius/hooks/block-dangerous-bash.js`：
