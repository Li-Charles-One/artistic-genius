...
// Trust gates project hooks. A project's .artistic-genius/settings.json can run
...
// root. The trust flag lives in user-global state (~/.artistic-genius/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.artistic-genius/settings.json) are the user's own and always run.

// TrustFilename is the user-global trust store under ~/.artistic-genius.
...
// TrustPath is ~/.artistic-genius/trust.json (homeDir overrides ~).
