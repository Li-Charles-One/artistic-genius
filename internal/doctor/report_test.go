...
func TestCollectReportRedactsSecrets(t *testing.T) {
	t.Setenv("ARTISTIC_GENIUS_TEST_SECRET", "sk-live-secret")
	...
	cfg.Providers = []config.ProviderEntry{{
		...
		Model:     "model-a",
		APIKeyEnv: "ARTISTIC_GENIUS_TEST_SECRET",
	}}
...
func TestCollectReportDoesNotRequireAPIKey(t *testing.T) {
	...
	if !strings.Contains(text, "artistic-genius 1.2.3 doctor") {
...
func TestRenderTextSurfacesWarningsUpTop(t *testing.T) {
	text := RenderText(Report{Warnings: []string{"config artistic-genius.toml: parse boom"}})
