...
const systemPrompt = "You are artistic-genius, a coding agent. Be concise and follow project conventions. " +
...
func TestReleaseCacheHitGuard(t *testing.T) {
	if os.Getenv("ARTISTIC_GENIUS_RELEASE_CACHE_GUARD") == "" {
		t.Skip("set ARTISTIC_GENIUS_RELEASE_CACHE_GUARD=1 to run the release cache guard")
	}

	threshold := envInt("ARTISTIC_GENIUS_CACHE_GUARD_THRESHOLD", 90)
	maxLowCases := envInt("ARTISTIC_GENIUS_CACHE_GUARD_MAX_LOW_CASES", 1)
	...
	if len(lows) > maxLowCases {
		...
		if os.Getenv("ARTISTIC_GENIUS_CACHE_GUARD_STRICT") != "" {
			t.Fatal(msg)
		}
	}
}
