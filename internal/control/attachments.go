...
// .artistic-genius/attachments and returns its repo-relative path for @referencing.
...
func cleanAttachmentPath(path string) (string, error) {
	...
	root := filepath.Join(".artistic-genius", "attachments")
	if clean == "." || clean == root || strings.HasPrefix(clean, ".."+string(filepath.Separator)) || !strings.HasPrefix(clean, root+string(filepath.Separator)) {
		return "", fmt.Errorf("attachment path is outside .artistic-genius/attachments")
	}
	if err := ensureAttachmentRoot(); err != nil {
		return "", err
	}
...
func attachmentPath(ext string) string {
	...
	return filepath.Join(".artistic-genius", "attachments", name)
}
