...
func TestClassifyRef(t *testing.T) {
	...
	files := map[string]bool{
		"src/main.go": true,
		"README.md":   true,
		".artistic-genius/attachments/clipboard-20260601-010203.000000.png": true,
		".artistic-genius/attachments/clipboard-20260601-010203.000000.yml": true,
		".artistic-genius/attachments/clipboard-20260601-010203.000000.zip": true,
	}
	...
	cases := []struct {
		...
		{".artistic-genius/attachments/clipboard-20260601-010203.000000.png", true, refImage},
		{".artistic-genius/attachments/clipboard-20260601-010203.000000.yml", true, refFile},
		{".artistic-genius/attachments/clipboard-20260601-010203.000000.zip", true, refFile},
...
func TestResolveRefsAttachmentKinds(t *testing.T) {
	temp := t.TempDir()
	attachmentsDir := filepath.Join(temp, ".artistic-genius", "attachments")
	if err := os.MkdirAll(attachmentsDir, 0o755); err != nil {
		t.Fatal(err)
	}
	ymlRef := filepath.ToSlash(".artistic-genius/attachments/config.yml")
	zipRef := filepath.ToSlash(".artistic-genius/attachments/archive.zip")
	pngRef := filepath.ToSlash(".artistic-genius/attachments/shot.png")
	if err := os.WriteFile(filepath.Join(temp, filepath.FromSlash(ymlRef)), []byte("name: artistic-genius\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(block, `<file path="`+ymlRef+`">`) || !strings.Contains(block, "name: artistic-genius") {
...
func TestWorkspaceImageRefsOnlyTreatAttachmentsAsImages(t *testing.T) {
	...
	attachment := filepath.Join(workspace, ".artistic-genius", "attachments", "shot.png")
	...
	if refs[1].kind != refImage || refs[1].path != ".artistic-genius/attachments/shot.png" {
