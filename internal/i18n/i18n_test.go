...
// $LC_ALL / $ARTISTIC_GENIUS_LANG. Unknown locales return "" so DetectLanguage falls
...
// TestDetectLanguagePriority verifies override beats env and that ARTISTIC_GENIUS_LANG
// beats LANG. With a clean env we fall back to English.
func TestDetectLanguagePriority(t *testing.T) {
	t.Setenv("ARTISTIC_GENIUS_LANG", "")
	t.Setenv("LC_ALL", "")
	...
	t.Setenv("ARTISTIC_GENIUS_LANG", "en")
	if got := DetectLanguage(""); got != "en" {
		t.Errorf("ARTISTIC_GENIUS_LANG=en overriding LANG=zh: got %q, want en", got)
	}
