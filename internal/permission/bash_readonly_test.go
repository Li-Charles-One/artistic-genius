...
func TestIsReadOnlyBashSubject(t *testing.T) {
	tests := []struct {
		...
		{"awk 'BEGIN{system(\"touch /tmp/artistic-genius-awk-test\")}'", false},
		{"awk 'BEGIN{print \"evil\" > \"/tmp/artistic-genius-awk-test\"}'", false},
		{"sed 's/a/b/' file.txt", false},
		{"sed -e 's/.*/touch \\/tmp\\/artistic-genius-sed-test/e' file.txt", false},
