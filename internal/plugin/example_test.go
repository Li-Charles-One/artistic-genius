...
// buildExamplePlugin compiles cmd/artistic-genius-plugin-example into a temp binary and
...
func buildExamplePlugin(t *testing.T) string {
	t.Helper()
	bin := filepath.Join(t.TempDir(), "artistic-genius-plugin-example")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	out, err := exec.Command("go", "build", "-o", bin, "artistic-genius/cmd/artistic-genius-plugin-example").CombinedOutput()
	if err != nil {
