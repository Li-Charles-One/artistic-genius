...
func TestHideWindowPreservesStdoutCapture(t *testing.T) {
	cmd := exec.Command("cmd", "/c", "echo", "artistic-genius-ok")
	HideWindow(cmd)
	out, err := cmd.Output()
	if err != nil {
		t.Fatalf("command failed: %v", err)
	}
	if !strings.Contains(string(out), "artistic-genius-ok") {
		t.Fatalf("output = %q, want it to contain artistic-genius-ok", out)
	}
}
