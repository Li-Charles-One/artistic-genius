...
func TestDocPathPrefersExisting(t *testing.T) {
	proj := t.TempDir()
	// An existing ARTISTIC_GENIUS.md should keep receiving notes (no split to AGENTS.md).
	if err := os.WriteFile(filepath.Join(proj, "ARTISTIC_GENIUS.md"), []byte("x"), 0o644); err != nil {
		t.Fatal(err)
	}
	set := Load(Options{CWD: proj})
	if got := set.DocPath(ScopeProject); filepath.Base(got) != "ARTISTIC_GENIUS.md" {
		t.Errorf("should append to the existing ARTISTIC_GENIUS.md, got %s", got)
	}
