...
func resetTree(repo string) {
	...
	_ = exec.Command("git", "-C", repo, "clean", "-fd", "-e", "artistic-genius.toml").Run()
}
...
func renderDiff(r diffReport) string {
	...
	fmt.Fprintf(&b, "## 🤖 Artistic Genius e2e — diff test-gen\n\n")
...
// repoRelFromModulePath turns "artistic-genius/internal/agent/foo.go" into
