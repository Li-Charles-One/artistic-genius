...
build:
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o bin/artistic-genius$(GOEXE) ./cmd/artistic-genius
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o bin/artistic-genius-plugin-example$(GOEXE) ./cmd/artistic-genius-plugin-example
...
cross:
	...
	@for p in darwin/amd64 darwin/arm64 linux/amd64 linux/arm64 windows/amd64 windows/arm64; do \
		...
		echo "build $$os/$$arch"; \
		CGO_ENABLED=0 GOOS=$$os GOARCH=$$arch go build -ldflags "$(LDFLAGS)" -o dist/artistic-genius-$$os-$$arch$$ext ./cmd/artistic-genius; \
	done
...
# layout: launcher at bin/codegraph/bin/codegraph beside bin/artistic-genius) and run the
# gated MCP end-to-end test against it. Requires `gh`. Windows: install via the
# upstream install.ps1 and run the test with ARTISTIC_GENIUS_CODEGRAPH_BIN set.
e2e-codegraph:
	...
	ARTISTIC_GENIUS_CODEGRAPH_E2E=1 ARTISTIC_GENIUS_CODEGRAPH_BIN=$$PWD/$$dest/bin/codegraph \
